...
public class DbCacheSizeTest extends TestBase {
    ...
    static final String[] COMMANDS = {
        ...
        /*11*/ "-records 100000 -key 10 -data 100 -orderedinsertion " +
               "-replicated -je.rep.preserveRecordVersion true",
        /*12*/ "-records 10000 -key 10 -data 20 " +
               "-offheap -maincache 9000000",
        /*13*/ "-records 100000 -key 10 -data 100 " +
               "-offheap -maincache 9000000",
        /*14*/ "-records 150000 -key 10 -data 100 " +
               "-offheap -maincache 9000000",
        /*15*/ "-records 150000 -key 10 -data 100 -duplicates " +
                "-offheap -maincache 9000000",
        /*16*/ "-records 10000 -key 10 -data 100 -duplicates " +
                "-offheap -maincache 9000000",
    };
    ...
    public void testSize() {
         ...
         * tolerance.
         */
        check("mainNoLNsOrVLSNs", util.getMainNoLNsOrVLSNs(),
              util.getMeasuredMainNoLNsOrVLSNs(), ERROR_ALLOWED);
        if (offHeap) {
            assertEquals(0, util.getMainNoLNsWithVLSNs());
        } else {
            check("mainNoLNsWithVLSNs", util.getMainNoLNsWithVLSNs(),
                  util.getMeasuredMainNoLNsWithVLSNs(), ERROR_ALLOWED);
        }
        check("mainWithLNsAndVLSNs", util.getMainWithLNsAndVLSNs(),
              util.getMeasuredMainWithLNsAndVLSNs(), ERROR_ALLOWED);
        check("offHeapNoLNsOrVLSNs", util.getOffHeapNoLNsOrVLSNs(),
            util.getMeasuredOffHeapNoLNsOrVLSNs(), ERROR_ALLOWED);
        check("offHeapWithLNsAndVLSNs", util.getOffHeapWithLNsAndVLSNs(),
            util.getMeasuredOffHeapWithLNsAndVLSNs(), ERROR_ALLOWED);
        ...
        check("noLNsOrVLSNsPreload", util.getMainNoLNsOrVLSNs(),
              util.getPreloadMainNoLNsOrVLSNs(), ERROR_ALLOWED);
        check("noLNsWithVLSNsPreload", util.getMainNoLNsWithVLSNs(),
              util.getPreloadMainNoLNsWithVLSNs(), ERROR_ALLOWED);
        check("withLNsAndVLSNsPreload", util.getMainWithLNsAndVLSNs(),
