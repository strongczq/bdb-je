...
//            System.out.println("Baseline LRU accuracy pct=" + pct + buf);
    ...
    private long getLocalCacheTotal(final int i) {
        ...
        long size = stats.getCacheTotalBytes();
        if (offHeapCache) {
            size += getDatabaseCacheBytes(dbs[i], true);
        }
        return size;
    }
...
//            " max free allowed = " + maxFreeMem);
    ...
    private void openOne(int i) {
        ...
        /*
         * With two caches, each having multiple LRU lists per LRUSet, the
         ...
         * per LRUSet (e.g., for the off-heap dirty set and non-dirty set).
         */
        if (offHeapCache) {
            envConfig.setConfigParam(
    ...
    private void checkEvenCacheUsage() {
        ...
        for (int i = 0; i < N_ENVS; i += 1) {
            ...
            final long val = getLocalCacheTotal(i);
